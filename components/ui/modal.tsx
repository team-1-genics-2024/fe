...
//   modelContainerClassName, // Custom class name for the modal content container
  ...
  return (
    ...
    > 
      <ModalContent
        className={
          "mx-4 my-6 rounded-xl border-4 bg-white p-0 shadow-none md:w-[30%] sm:mx-6 sm:my-8 sm:w-full sm:max-w-xl sm:p-0"
        }
      >
        {children}
      </ModalContent>
...
//   crossClassName, // Custom class name for the cross icon
  ...
  return (
    <ModalHeader
      className={
        "flex items-center justify-between rounded-t-xl border-1.5 border-b-0 border-typo-outline-2 bg-typo-white text-lg font-semibold text-typo-main md:text-xl"
      }
      {...rest}
    >
      {children}
      <button
        type="button"
        className={
          "absolute right-0 top-0 float-right mr-2 mt-3 text-typo-main hover:opacity-70"
        }
...
function Body({  children, ...rest }: ContentProps & ModalBodyProps) {
  return (
    <ModalBody
      className={
...
//   className,
  ...
  return (
    <ModalFooter
      className={"flex flex-col rounded-b-xl border-1.5 border-t-0 border-typo-outline-2 bg-typo-white text-typo-main"}
      {...rest}
    >
      {children}
    </ModalFooter>
